...
def get_remote_url(repo_name):
    if not GITHUB_USERNAME or not GITHUB_TOKEN:
        return (
            None,
            "CRITICAL ERROR: GITHUB_USERNAME or GITHUB_TOKEN not found. Please check your .env file.",
        )
...
def create_github_repo(repo_name, is_private):
    ...
    print(
        f"Creating new {'private' if is_private else 'public'} repository on GitHub..."
    )
...
def register_project(project_path):
    ...
    if error == "NOT_FOUND":
        ...
        should_create = (
            input(
                "Would you like to create a new repository on GitHub with this name? [y/n] "
            )
            .lower()
            .strip()
        )
        ...
        if should_create in ["y", "yes", ""]:
            visibility = (
                input("Should the repository be private? [y/n] ").lower().strip()
            )
            ...
            if not is_git_repo:
                subprocess.run(
                    ["git", "init"], cwd=local_path, capture_output=True, check=False
                )

            subprocess.run(
                ["git", "remote", "add", "origin", str(remote_url)],
                cwd=local_path,
                capture_output=True,
                check=False,
            )
    ...
    projects[project_nickname] = {"local_path": local_path, "remote_url": remote_url}
...
def get_navigation_command(nickname):
    ...
    try:
        ...
        if nickname in projects:
            ...
            print(
                f"\033[91m\033[1mError: Project nickname '{nickname}' not found in registry.\033[0m"
            )
            return None
    except FileNotFoundError:
        print(
            "\033[91m\033[1mError: Project registry not found. Please register a project first.\033[0m"
        )
        return None
